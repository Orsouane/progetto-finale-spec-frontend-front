...
function Comparator({ GameToShow }) {
    ...
    useEffect(() => {
     ...
     //! Gestione dei due Giochi da confrontare
     //* 1° Giocho
               ...
               <div className='flex flex-col '>
                    ...
                    <div className='flex  gap-3 sm:gap-5 justify-center items-center '>
                         ...
                         <select onChange={HandleFirstGame}
                              ...
                              className='bg-[#1F2937] rounded-md text-[#8E95A2] border border-[#8E95A2]  text-xs    h-8  cursor-pointer max-w-27   '>
                              <option value="">{compare}</option>

                              {
                                   GameToShow.map((element, index) => <option value={element.title} key={index}>{element.title}   </option>
                         ...
                         <button onClick={handleCompare} className=' my-1     rounded-md text-xs  r w-fit  bg-[#1F2937]  text-[#8E95A2] border border-[#8E95A2] hover:text-white hover:bg-[rgb(129,110,216)] p-1.5 cursor-pointer'>Compare</button>
                    </div>
               </div>
